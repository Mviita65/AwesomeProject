...
const checkHighScores = (
    ...
    for (let i=0;i<10;i++){     // to check all highScores and get the one for rewriting
        if (tempScores[i].score < clickCounter && trackHighScore && check===0){ 
            check = 1                   // enough clicks to rewrite highscores found
            setTrackHighScore(false)
            let oldScore = tempScores[i].score  // old highScore value to pass downward
            ...
            setHighScores(tempScores)           // work copy to replace highscorelist
        }
    }
}
